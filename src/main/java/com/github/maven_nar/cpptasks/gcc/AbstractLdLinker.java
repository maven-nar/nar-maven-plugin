...
public abstract class AbstractLdLinker extends CommandLineLinker {
    ...
    protected AbstractLdLinker(String command, String identifierArg,
        ...
        String outputSuffix, boolean isLibtool, AbstractLdLinker libtoolLinker) {
    ...
    protected String[] addLibrarySets(CCTask task, LibrarySet[] libsets,
        ...
        for (int i = 0; i < libsets.length; i++) {
            ...
            if (libdir != null) {
                ...
                if (currentDir != null && currentDir.getParentFile() != null) {
                    relPath = CUtil.getRelativePath(currentDir.getParentFile()
                        .getAbsolutePath(), libdir);
                }
                if (set.getType() != null
        ...
        if ((previousLibraryType != null)
            && previousLibraryType.getValue().equals("static") && !isDarwin()) {
            endargs.addElement(getDynamicLibFlag());
        }
        // ENDFREEHEP
    ...
    public String[] getLibraryPatterns(String[] libnames,
        ...
        if (libType != null && "framework".equals(libType.getValue())
            && isDarwin()) {
            for (int i = 0; i < libnames.length; i++) {
                buf.setLength(0);
                buf.append(libnames[i]);
            ...
            if (libType == null || !"static".equals(libType.getValue())) {
                if (isHPUX()) {
                    offset = addLibraryPatterns(libnames, buf, "lib", ".sl",
                        patterns, offset);
                } else {
                    offset = addLibraryPatterns(libnames, buf, "lib", ".so",
                        patterns, offset);
                }
            }
        }
